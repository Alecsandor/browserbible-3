...
function generate(inputBasePath, info, createIndex, startProgress, updateProgress) {
	var breakChar = '\n';
	// LOAD strongs
	var strongsPath = filePath = path.join(inputBasePath, 'strongs.json'),
	...
	for (var strongsNumber in strongsData) {
		...
		if(strongsNumber === "G976") console.log(strongsEntry.lemma)
	}
	// console.log(strongsLemmaKey)
		...
		for (var i = 0, il = lines.length; i < il; i++) {
			...
			switch (usfm.key) {
				...
				case 'v':
					...
					var formatted = usfmParser.formatText(usfm.text, noteNumber, chapterVerse);
					notes += formatted.notes;
					let words = formatted.text.split(" ");
					t = words.map(w => w.replace(/\"|,|\./g, "").toLocaleLowerCase());
					// console.log(strongsLemmaKey[t[0]]);
					words = words.map(w => {
						let parsedWord = w.replace(/\"|,|\./g, "");
						console.log(parsedWord);
						return `<l s="${strongsLemmaKey[parsedWord.toLocaleLowerCase()]}" m="V-3AAIS">${w}</l>`
					});
					// console.log(strongsLemmaKey["Βίβλος"]);
